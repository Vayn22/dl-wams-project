...
export default function DoctorModelsPage() {
  ...
  const myPatients = useMemo(
    () => getPatientsData().filter((p) => p.assignedDoctorId === currentUser?.id),
    [currentUser, getPatientsData]
  );

  if (specialty === "radiologiste" || specialty === "généraliste") {
    return (
      <PageTransition className="flex min-h-[70vh] flex-col items-center justify-center text-center">
        ...
        <p className="mt-2 max-w-sm text-slate-400">{"Votre spécialité ne dispose pas encore de modèles d'intelligence artificielle."}</p>
        <Button variant="outline" className="mt-4">{"Contacter l'administrateur"}</Button>
      </PageTransition>
    );
  }
        ...
        <div className="flex items-start justify-between bg-gradient-to-r from-purple-600 to-purple-800 p-5">
          <div>
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-white">NeuroScan Vision</h2>
            </div>
            <p className="mt-2 text-sm text-purple-200">{"Détection et localisation de tumeurs cérébrales par analyse d'image IRM"}</p>
          </div>
          <div className="text-right">
            <Badge className="border border-white/30 bg-white/20 text-white">Actif</Badge>
            <p className="mt-2 text-xs text-purple-200">v2.4.1</p>
          </div>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg bg-purple-50 p-4">
                ...
                <ul className="space-y-1 text-sm text-slate-700">
                  <li>• Zone suspecte détectée dans le lobe frontal droit (probabilité 87%)</li>
                  <li>• Légère asymétrie hémisphérique observée</li>
                  <li>• Aucune hémorragie détectée</li>
                </ul>
        ...
        <div className="flex items-start justify-between bg-gradient-to-r from-slate-700 to-slate-900 p-5">
          <div>
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-white">NeuroText Assist</h2>
            </div>
            <p className="mt-2 text-sm text-slate-300">{"Aide au diagnostic différentiel et analyse des symptômes neurologiques"}</p>
          </div>
          <div className="text-right">
            <Badge className="border border-white/30 bg-white/20 text-white">Actif</Badge>
            <p className="mt-2 text-xs text-slate-400">v1.9.0</p>
          </div>
        </div>
        ...
        <div className="space-y-4 p-6">
          ...
          <Button
            ...
            onClick={async () => {
              ...
              setTextResult("Sur la base des symptômes décrits, plusieurs diagnostics différentiels sont à considérer : migraine avec aura (probabilité élevée), hypertension intracrânienne (à explorer), névrite optique...");
            }}
          >
